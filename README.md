...
`-- lib/
    ...
    |-- contact-link.ts    # Pre-fill form kontak via sessionStorage (URL tetap bersih)
...
4. **"Alternate page with proper canonical tag" / "Crawled - currently not indexed" untuk URL dengan query string** — varian homepage atau detail yang HTML-nya identik dengan kanoniknya (mis. `/?contactSubject=…&contactBody=…` warisan, atau `/proyek/x?lang=…`). Ini **bukan error** (kanoniknya benar dan Google sengaja tidak mengindeks duplikat), tapi tetap boros crawl budget dan mengaburkan data GSC. Penanganan: pre-fill form kontak sekarang dibawa via **sessionStorage** (`src/lib/contact-link.ts`) tanpa mengubah URL; varian lama di-redirect **308** ke `/#kontak` oleh `src/proxy.ts` (query `?lang=` dipertahankan); varian `?lang=` tidak lagi diiklankan di sitemap/hreflang karena i18n 100% client-side. Verifikasi: `curl -sI "https://sigitadi.id/?contactSubject=x"` harus `308` ke `https://sigitadi.id/#kontak`. Setelah deploy, entri ini menyusut saat Google me-recrawl — URL yang kini merespons 308 bukan lagi "alternate page". Untuk halaman baru yang sehat, "not indexed" memang masalah waktu (hari–minggu) — pastikan `lastmod` di `sitemap.xml` segar dan URL ter-submit.
5. **Canonical** — semua rute publik canonical ke URL **tanpa query** (`localeAlternates()` di `src/lib/seo.ts`). Karena `?lang=id`/`?lang=en` menghasilkan HTML identik (i18n client-side), varian itu **tidak diiklankan** lagi sebagai alternate sitemap/hreflang — jangan submit `?lang=` sebagai URL terpisah. `?lang=` tetap berfungsi sebagai deep-link/toggle bahasa.
