...
export const alt = "Artikel — Sigit Web Porto";
...
export default async function Image({ params }: Props) {
  ...
  const title = article ? article.title : "Artikel Tidak Ditemukan";
  const subtitle = article
    ? (article.summary || article.content).slice(0, 90)
    : "Artikel yang Anda cari belum dipublikasikan atau URL tidak sesuai.";
  const imageUrl = article?.imageUrl && /^https?:\/\//.test(article.imageUrl) ? article.imageUrl : undefined;
    ...
    (
      <div style={{ background: SHELL.bg, width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between", color: "#f8fafc", padding: "48px", position: "relative", overflow: "hidden", fontFamily: "sans-serif" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", position: "relative" }}>
          <div style={{ ...SHELL.wrap, borderRadius: "8px", padding: "8px 16px", fontSize: "20px", fontWeight: 700, fontFamily: "monospace", letterSpacing: "1px" }}>SIGITOS // ARTICLE</div>
