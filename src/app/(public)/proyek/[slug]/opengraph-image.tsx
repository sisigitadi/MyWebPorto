...
export const alt = "Proyek — Sigit Web Porto";
...
export default async function Image({ params }: Props) {
  ...
  const title = project ? project.title : "Proyek Tidak Ditemukan";
  const subtitle = project
    ? (project.summary || project.description).slice(0, 90)
    : "Proyek yang Anda cari belum dipublikasikan atau URL tidak sesuai.";
  const imageUrl = project?.thumbnailUrl && /^https?:\/\//.test(project.thumbnailUrl) ? project.thumbnailUrl : undefined;
    ...
    (
      <div style={{ background: SHELL.bg, width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between", color: "#f8fafc", padding: "48px", position: "relative", overflow: "hidden", fontFamily: "sans-serif" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", position: "relative" }}>
          <div style={{ ...SHELL.wrap, borderRadius: "8px", padding: "8px 16px", fontSize: "20px", fontWeight: 700, fontFamily: "monospace", letterSpacing: "1px" }}>SIGITOS // PROJECT</div>
