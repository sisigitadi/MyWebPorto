...
export const alt = "Produk Digital — Sigit Web Porto";
...
export default async function Image({ params }: Props) {
  ...
  const product = products.find((p) => p.published && (p.slug === slug || p.id === slug));

  const title = product ? `${product.title} — Store.zip` : "Produk Tidak Ditemukan";
  const subtitle = product ? product.description.slice(0, 90) : "Produk yang Anda cari belum dipublikasikan.";
  const imageUrl = product?.thumbnailUrl && /^https?:\/\//.test(product.thumbnailUrl) ? product.thumbnailUrl : undefined;
    ...
    (
      <div style={{ background: SHELL.bg, width: "100%", height: "100%", display: "flex", flexDirection: "column", justifyContent: "space-between", color: "#f8fafc", padding: "48px", position: "relative", overflow: "hidden", fontFamily: "sans-serif" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", position: "relative" }}>
          <div style={{ ...SHELL.wrap, borderRadius: "8px", padding: "8px 16px", fontSize: "20px", fontWeight: 700, fontFamily: "monospace", letterSpacing: "1px" }}>SIGITOS // PRODUCT</div>
