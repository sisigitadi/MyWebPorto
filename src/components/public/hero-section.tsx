...
export function HeroSection({ profile }: HeroSectionProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[350px] bg-primary/5 rounded-full blur-3xl pointer-events-none -z-10" />

      <div className="max-w-6xl mx-auto px-4 sm:px-6 w-full">
        {/* Availability Pill */}
        {profile.availableForHire && (
          <div className="hero-status inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-medium border border-border bg-muted/40 text-foreground mb-8">
        ...
        <div ref={ctaRef} className="flex flex-wrap items-center gap-4 pt-8">
          <Button asChild size="lg" className="h-12 px-7 font-medium gap-2 text-sm shadow-sm">
            <Link href="/proyek">
          ...
          <Button asChild variant="outline" size="lg" className="h-12 px-7 font-medium gap-2 text-sm border-border bg-card hover:bg-muted/40">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-16 max-w-4xl border-t border-border/40 mt-16">
          {profile.stats.map((stat, idx) => (
            <div
              key={idx}
              className="hero-stats-item p-4 rounded-xl border border-border/60 bg-card/40 text-left transition-transform hover:-translate-y-0.5 duration-200"
            >
        ...
        <div className="pt-8 flex flex-wrap items-center gap-2">
          ...
          {profile.skills.map((skill) => (
            <Badge
              key={skill}
              variant="outline"
              className="font-normal text-xs py-1 px-3 bg-background border-border text-foreground hover:border-foreground/30 transition-colors"
            >
              {skill}
            </Badge>
          ))}
